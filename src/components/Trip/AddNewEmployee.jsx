  ...
  const addEmployeeForm = async () => {
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        const docRef = doc(db, 'RouteList', company, date, 'All', type, routeId, 'Employees', EmpID);
        setDoc(docRef, employeeData);

        const docRefs = doc(db, 'EmployeeList', EmpID);
        setDoc(docRefs, employeeData);
        // Loop through history documents and update EmpName

        const historyDocRefs = doc(db, "EmployeeList", EmpID, "HistoryCancelled", "ZFlag");
        setDoc(historyDocRefs, { Flag: "" });
        const historyDocRefss = doc(db, "EmployeeList", EmpID, "HistoryCompleted", "ZFlag");
        setDoc(historyDocRefss, { Flag: "" });
